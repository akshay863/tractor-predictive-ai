...
print("Generating 15-Primary Sensor Telemetry Data (70/30 Balanced Split)...")
np.random.seed(42)
n_samples = 3000
...
# 2. Inject 900 Faults (225 of each DTC) for a perfect 70/30 split
df.loc[1800:2024, 'Hydraulic_Pressure_bar'] = np.random.normal(120, 15, 225)
df.loc[1800:2024, 'Hydraulic_Flow_Lpm'] = np.random.normal(45, 5, 225)
df.loc[1800:2024, 'Failure_Code'] = 1 

df.loc[2025:2249, 'Coolant_Temp_C'] = np.random.normal(118, 4, 225)
df.loc[2025:2249, 'Exhaust_Gas_Temp_C'] = np.random.normal(680, 20, 225)
df.loc[2025:2249, 'Failure_Code'] = 2 

df.loc[2250:2474, 'Transmission_Temp_C'] = np.random.normal(115, 5, 225)
df.loc[2250:2474, 'Wheel_Slip_pct'] = np.random.normal(30, 4, 225)
df.loc[2250:2474, 'Draft_Load_kN'] = np.random.normal(45, 5, 225)
df.loc[2250:2474, 'Failure_Code'] = 3 

df.loc[2475:2699, 'Battery_Voltage_V'] = np.random.normal(11.0, 0.3, 225)
df.loc[2475:2699, 'Fuel_Rail_Pressure_bar'] = np.random.normal(900, 50, 225)
df.loc[2475:2699, 'Failure_Code'] = 4 

df.to_csv('industry_tractor_telemetry.csv', index=False)
print("Success: 70/30 balanced dataset built.")